...
class AuthControllerUnitTests {
    ...
    void register_Success_Returns201() throws Exception {
        when(userService.registerUser(anyString(), anyString(), anyString(), anyString(), any(Role.class)))
                .thenReturn(mockUser);
        ...
        verify(userService, times(1)).registerUser(anyString(), anyString(), anyString(), anyString(), any(Role.class));
    }
    ...
    void register_UsernameExists_Returns409() throws Exception {
        when(userService.registerUser(anyString(), anyString(), anyString(), anyString(), any(Role.class)))
                .thenThrow(new RuntimeException("Username 'testuser' is already taken. Please choose another."));
        ...
        mockMvc.perform(post("/auth/register")
                ...
                .andExpect(jsonPath("$.message").value("Username 'testuser' is already taken. Please choose another."));
    ...
    @Test
    @DisplayName("GET /api/auth/check-username - Should return true when username is available")
    ...
    @Test
    @DisplayName("GET /api/auth/check-username - Should return false when username is taken")
